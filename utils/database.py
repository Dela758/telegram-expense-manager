...
class ExpenseDB:
    ...
    def _get_conn(self):
        # Return the context manager. WAL mode will be set in methods.
        return aiosqlite.connect(self.db_path, timeout=20)
    ...
    async def add_subscription(self, user_id, name, amount, category, billing_cycle, start_date=None):
        if not start_date: start_date = datetime.now().date().isoformat()
        async with self._get_conn() as conn:
            await self._init_conn(conn)
            await conn.execute("""
                INSERT INTO subscriptions (user_id, name, amount, category, billing_cycle, start_date) 
                VALUES (?, ?, ?, ?, ?, ?)
            """, (user_id, name, amount, category, billing_cycle, start_date))
            await conn.commit()

    async def get_subscriptions(self, user_id):
        async with self._get_conn() as conn:
            await self._init_conn(conn)
            cursor = await conn.execute("SELECT * FROM subscriptions WHERE user_id = ?", (user_id,))
            rows = await cursor.fetchall()
            return [dict(row) for row in rows]

    async def update_subscription_log(self, sub_id, last_logged):
        async with self._get_conn() as conn:
            await self._init_conn(conn)
            await conn.execute("UPDATE subscriptions SET last_logged = ? WHERE id = ?", (last_logged, sub_id))
            await conn.commit()

    async def delete_subscription(self, sub_id, user_id):
        async with self._get_conn() as conn:
            await self._init_conn(conn)
